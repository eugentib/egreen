...
function new_device(message_str) {
	var msg_arr = topic_split(message_str);
	...
	if (!topics.includes(message_str)) {
		...
		mqttclient.subscribe('Kaufland/' + msg_arr[1] + '/sw', mqtt_subscribe);
		mqttclient.subscribe('Kaufland/' + msg_arr[1] + '/FW_ver', mqtt_subscribe);
		mqttclient.subscribe('Kaufland/' + msg_arr[1] + '/crash', mqtt_subscribe);
		mqttclient.subscribe('Kaufland/' + msg_arr[1] + '/rssi', mqtt_subscribe);
		mqttclient.subscribe('Kaufland/' + msg_arr[1] + '/scr', mqtt_subscribe);
		mqttclient.subscribe('Kaufland/' + msg_arr[1] + '/nrbalot', mqtt_subscribe);
		mqttclient.subscribe('Kaufland/' + msg_arr[1] + '/model', mqtt_subscribe);
		mqttclient.subscribe('Kaufland/' + msg_arr[1] + '/vsw', mqtt_subscribe);
	}
}
...
function get_scroll(mac, txt) {
	if (mac in scroll) {
		...
		if (pos >= 0) {
			...
			if (len < 111) scroll[mac] = temp.slice(0, pos) + txt + temp.slice(pos + 16);
		...
		msg_log.log[mac] = fs.createWriteStream("./log/scroll_" + mac + ".txt", { flags: 'a' })
		msg_log.log[mac].write(timp() + scroll[mac] + "\n");
	}
...
function mqtt_messsageReceived(topic, message, packet) {
	if (message.length == 34) {
		...
		var message_str = message.toString(); //convert byte array to str

	var topic_arr = topic_split(topic);

	//    mqtt_msg.emit('new_msg',topic_arr, message_str);

	if ((topic_arr[0] != "Kaufland") && (topic_arr[0] != "LWT")) { console.log('Topic neinteresant :', topic_arr); return; }
		...
		if (topic_arr[2] == 'lcd') {
			if (mac == 'x80646FA929F3') {
				...
				if ((message_str.slice(0, 16) == 'PRESA E OK     K')
					...
					&& (message_str.slice(16, 30) != 'SENSOR FAULT !')) get_scroll(mac, message_str.slice(16));
				else log_message(mac, message_str);
			}
		}
		if (topic_arr[2] == 'scr') {
			if (!(mac in msg_log.scr)) {
				msg_log.scr[mac] = fs.createWriteStream("./log/dev_scroll_" + mac + ".txt", { flags: 'a' })
				msg_log.scr[mac].write(timp() + message_str + "\n");
			} else msg_log.scr[mac].write(timp() + message_str + "\n");
		}
		mqtt_msg.emit('new_msg', JSON.parse(JSON.stringify(topic_arr)), JSON.parse(JSON.stringify(message_str)));
	}
};
