...
function save_check() {
    ...
    $(this).parents('tr').find(`td:gt(${col})`).each(function () {
...
function insert_row_dturi(date, dataset) {
    var adate = ['idt', 'serie', 'model', 'localitate', 'nrmag', 'dataRevizie', 'intervalRevizie', 'intervalBaloti', 'email_serv', 'email_kauf'];
    // console.log(date)
    //   console.log(dataset)
...
function array_to_dturi(inregistrari) {        //Se populeaza tabelul din procesare cu datele venite de la server
    ...
    //    $("#dt_tabel tbody tr").on('dblclick', load_dturi_form_data);
}
...
function array_to_l2(inregistrari) {        //Se populeaza tabelul din procesare cu datele venite de la server
    ...
    $("#l2_tabel tbody").empty();
    for (var i = 0; i < inregistrari.length; i++) {
        if ((i % 2) == 0) $('#l2_tabel tbody').append(`<tr><td style="border: none;"></td></tr><tr><td style="border: none;"></td></tr>`);
...
function insert_row_l2(date, i) {
    ...
    var row = '<tr class="ns" id="' + date.id + '" ><td data-filtru="fckdt" style="width: 10%;"><i class="fa fa-pencil" aria-hidden="true"></i></td>';
...
function get_checked_dts() {
    arr = [];
    $('#dt_tabel .ckbox:checkbox:checked').each((i, e) => { arr.push($(e).closest('tr').find("td:eq(2)").html()); });
    var msg = "Sterge echipamentul cu seria:";
    if (arr.length > 1) msg = "Sterge echipamentele cu seriile:";
    if (arr.length) $('<div style="text-align:center;"></div>').html(msg + arr.join()).dialog({
        title: "Atentie!",
        resizable: false,
        modal: true,
        width: 500,
...
function get_checked_dt() {
    ...
    if (arr.length > 1) {
        ...
        if (arr.length > 0) {
            ...
            $('#dt_tabel .ckbox:checkbox:checked').parents('tr').find('td:gt(2)').each(function () {
                var html = $(this).html();
