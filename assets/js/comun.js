...
setInterval(function () { $("#clock").html(datasiora()); }, 1000);
...
function change_password() {
    ...
    $('<div id="pwd_ch" style="max-width: 300px;><div">Parola veche<br><input type="password" style="z-index:10000" name="old_pass"><br>' +
        'Parola noua<br><input type="password" style="z-index:10000" name="new_pass"></br><div></div>').dialog({
            modal: true,
            "width": 450,
            "height": 300,
        ...
        switch (received.content) {
            ...
            case "preluare_autocomplet":
                if (received.data.length > 0) {
                    ...
                    if (received.dest == "oras") $('#oras').autocomplete({ source: array });
                    if (received.dest == "client") $('#nume_client').autocomplete({ source: array });
                }
                break;
            default: ws_msg(received);
        }
    }
...
function getCurrentDateTimeMySql(acum) {
    var localISOTime = (new Date((acum ? acum : Date.now()) - tzoffset)).toISOString().slice(0, 19).replace('T', ' ');
...
function check_table() {             // Se de/biveaza tabelul
    ...
    for (var i = 0; i < t.childNodes.length; i++) {
        if ($(t.children[i]).is(":visible")) t.children[i].children[0].children[0].checked = this.checked;
    }
}
...
function custom_alert(message, title) {
    ...
    $('<div style="text-align:center;"></div>').html(message).dialog({
        title: title,
        resizable: false,
        modal: true,
        width: 500,
...
function datepicker2mysqldate(ref) {
    let data = $(ref).val().split(/[- .:]/);
    return [data[2], '-', data[1], '-' + data[0], ' ', data[4], ':', data[5]].join('');
}
...
function request_data() {
    ...
    if (typeof raportari_filtre === 'boolean') smsg.command = 'get_from_db_raportari_filtre';
...
$('#datepicker').val((new Date(firstDay)).toLocaleDateString('ro-RO').substring(0, 10).replace(/\//g, ".") + " ora 00:00");
//$('#datepicker2').val((new Date(Date.now())).toLocaleDateString('ro-RO').substring(0, 10).replace(/\//g, ".") + " ora 23:59");
$('#datepicker2').val((new Date(lastDay)).toLocaleDateString('ro-RO').substring(0, 10).replace(/\//g, ".") + " ora 23:59");
...
function sort() {
    ...
    var rows = table.find('tbody tr:not([style*="display: none"])').toArray().sort(comparer($(this).index()))
    this.asc = !this.asc
    if (!this.asc) { rows = rows.reverse() }
    for (var i = 0; i < rows.length; i++) {
        $(rows[i]).find('[data-filtru=fid]').text(i + 1); // Reindexeaza tabelul
...
//        console.log('row = ', rows[i]);
        ...
        $(rows[i]).find('[data-filtru=fid]').text(index++); // Reindexeaza tabelul
        $(rows[i]).removeClass("odd even");
        if (i % 2) $(rows[i]).addClass("odd");
        else $(rows[i]).addClass("even");
        table.append(rows[i]);
...
function filtreaza() {
    ...
    focus_id = this.id;
    console.log('focus_id = ', focus_id); console.log('target_id = ', target);
    var autoarr = $('#' + target + ' tr td:nth-child(' + (this.parentNode.cellIndex + 1) + ')').map(function () {
        return $(this).text()
    }).get();
